...
async def search(req: WrongNoteSearchRequest, _=Depends(verify_internal)):
...
async def embed(req: WrongNoteEmbedRequest, _=Depends(verify_internal)):
    """저장된 오답노트를 ChromaDB에 인덱싱 (F-14)."""
    out = chroma.embed_note(req.user_id, req.wrong_note_id, req.content)
    return WrongNoteEmbedResponse(status=LLMStatus.success, **out)


@router.post("/ask", response_model=NoteAskResponse)
async def ask(req: NoteAskRequest, _=Depends(verify_internal)):
