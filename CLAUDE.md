...
- [x] **5대 필수 산출물(`01`~`05`)로 통합**, `NN_제목.md` 네이밍 통일. 검증가이드 → `05` 부록 A, 추적성 매트릭스 → `01` §10으로 **녹여 통합**(옛 06·07 파일 제거).
- [x] 외부 제출용 톤 정리(초안/배점/정리대상/고아 등 **내부 코멘트 제거**). S3 **미도입 확정** — 프론트 static을 S3로 이전하는 것만 확장 포인트로 기재.
- [x] **평가 피드백 반영**(무편향 평가 에이전트 기준): 추적성 매트릭스(FR↔화면↔시나리오↔테스트, `01` §10), 화면설계서 스크린샷 임베드·브레이크포인트, 요구사항 유스케이스(UC)·OpenAI I/O 스키마, **LLM 예외 테스트**(ai_proxy 타임아웃·HTTP·형식오류·stub 4건) 추가 → Django 41건.
- [x] **배포 자동화 정리**: 옛 `deploy.yml`(GitHub Actions, feature 브랜치·없는 compose 참조)을 **수동 전용·이력 보존**으로 정리(참조값 `main`·`compose/main·worker`로 현행화). 실제 배포는 `deploy/*.sh`로 일원화 — 인수기준 #8·테스트보고서 개선이력에 반영. 남은 감점 요인은 **실제 AWS 라이브 구동 증적**(사용자 캡처).
