...
- [x] **평가 피드백 반영**(무편향 평가 에이전트 기준): `07_추적성_매트릭스.md`(FR↔화면↔시나리오↔테스트) 신설, 화면설계서에 스크린샷 임베드·브레이크포인트 치수, 요구사항에 유스케이스(UC)·OpenAI I/O 스키마, **LLM 예외 테스트**(ai_proxy 타임아웃·HTTP·형식오류·stub 4건) 추가 → Django 41건. 남은 감점 요인은 **실제 AWS 라이브 구동 증적**(사용자 캡처 필요).
